...
const DEFAULT_PROPS = {
  ...
  slides: [
    {
      description:
        "…a gente tem se sentido muito satisfeitos com o trabalho prestado pela we.digi…entregas consistentes…pessoal muito criativo. Conseguiram melhorar muito a qualidade da nossa loja…temos três lojas hoje…",
      avatar:
...
function SliderItem(
  { slide, id, mark }: { slide: Testimonial; id: string, mark: string },
) {
    ...
    <>
      ...
      hx-on:click={useScript((total) => {
        ...
        {slides?.map((slide, index) => {
          return (
          <li class="carousel-item [&_button]:[filter:grayscale(1);] [&_button:disabled]:[filter:none]">
...
function Buttons(props: {total: number}) {
    ...
    >
      ...
      >
        {slides?.map((slide, index) => (
          <Slider.Item
            index={index}
            class="carousel-item max-w-[1520px] w-full"
          >
            <SliderItem
              slide={slide}
              mark={mark}

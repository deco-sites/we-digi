...
import isImageValidServerSide from "site/utils/imageValidation.ts"
...
const CONTENT_STYLES =
  `max-w-5xl mx-auto ${PARAGRAPH_STYLES} ${HEADING_STYLES} ${CODE_BLOCK_STYLES} ${IMAGE_STYLES} ${BLOCKQUOTE_STYLES}`;

const DEFAULT_AVATAR =
  "https://decoims.com/we-digi/5f31c2a6-c7ad-43f9-b2cb-ae954c7661f7/7286de42_7286de42-e9c5-4fcb-ae8b-b992eea4b78e.png";
  ...
  return (
    <div className="px-4 md:px-0 py-12 lg:py-28 container mx-auto">
      ...
      <div className="w-full flex flex-col gap-20">
        ...
        <div class="flex flex-col gap-10 max-w-5xl w-full mx-auto">
          {/*<div class="space-y-4">
...
export const loader = async (props: Props, _req: Request, _ctx: AppContext) => {
  const originalImageUrl = props?.page?.post?.image

  if (originalImageUrl) {
    const isValid = await isImageValidServerSide(originalImageUrl);

    if (!isValid) {
      const modifiedProps = JSON.parse(JSON.stringify(props));

      if (modifiedProps.page && modifiedProps.page.post) {
        modifiedProps.page.post.image = "";
      } else {
        return { ...props };
      }

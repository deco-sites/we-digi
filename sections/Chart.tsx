...
export interface CTA {
    ...
    href: string;
    text: string;
    type?: 'default' | 'outline' | 'inline' | 'rounded';
    outline: boolean;
}
...
export default function Chart({ title, tag, sideImage, background, description, cta = [
    { id: "change-me-1", href: "/", text: "CONHEÇA NOSSOS SERVIÇOS", outline: false }], tech }: Props) {
    ...
    //   });
    // const fadeItems = (texts) => addEventListener("load", () => {
    //     console.log(texts);
    ...
    // });
    return (<section class={`${tech ? "font-effraTrial" : ""} flex items-center justify-center bg-custom-size`}  style={{ backgroundImage: `url(${background})`, backgroundRepeat: 'no-repeat', backgroundSize: 'cover' }}>
      <div class="container mx-auto px-4 py-6 md:pb-28 md:pt-28 lg:flex lg:items-center lg:gap-8 2xl:max-w-[1267px]">
        {/* Bloco de Texto */}
        <div class="lg:w-1/2">
          <div class="w-full flex flex-col mb-4 lg:mb-8 max-w-[550px]">
            {tag && <p class="text-[#76F5F7] text-base" >{tag}</p>}
            {title && <p class="text-2xl lg:text-[40px]/[50px]" style={{ color: title.color ?? '#ababab'}}>{title.text}</p>}
          </div>
            <div class="flex flex-col items-start justify-center gap-3">
              ...
              */}
              <div id="textElement" class="mb-4 lg:mb-8 max-w-[550px] [&_span]:[line-height:24px] [&_span]:[display:flex] [&_ul]:[list-style:disc] [&_ul]:pl-5 flex flex-col text-sm lg:text-base">
                <p class="textElement__title" style={{ color: description?.color }}>{description?.title}</p>
                {description?.label && (<span class="flex-col text-white [&_ul]:pl-8" dangerouslySetInnerHTML={{ __html: description?.label }}></span>)}
              </div>
              {/* <script
                  type="module"
                  defer
            ...
            <div class="flex items-center gap-3 mb-4 lg:mb-0">
              {cta?.map((item) => (<a key={item?.id} id={item?.id} href={item?.href} target={item?.href.includes("http") ? "_blank" : "_self"} class={`font-effraTrial text-white btn ${item.type == "outline" ? "btn-outline outline-arrow" :
                item.type == "inline" ? "btn-inline inline-arrow" : item.type == "rounded" ? "btn-rounded" : "btn-primary"}`}>
                  {item?.text}
                </a>))}
            </div>
        </div>
        ...
        <div class="lg:w-1/2 mt-2 lg:mt-0 flex justify-center items-center aspect-square">
          <div id="chart-image" class="w-fit">
